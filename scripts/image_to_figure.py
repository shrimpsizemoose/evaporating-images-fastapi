...
def quantize_colors(
    ...
    return [rgb_to_hex(tuple(color)) for color in colors]


def resize_and_pixelate(
    image_path,
    grid_size=25,
):
    img = Image.open(image_path).convert("RGB")
...
def create_figure_json(
    ...
    # convert mapped indices to letters
    points = []
    for row in mapped:
        point_row = []
        for val in row:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python image_to_figure.py <image_path> [output_path] [grid_size] [num_colors] [name]"
        )
        print("\nExample:")
        print(
            "  python image_to_figure.py flower.png figures/flower.json 25 8 'queen_of_night'"
        )
        print("\nDefaults:")
